...
3. **Just 86 locations cover 50% of all violations.** The coverage curve shows **86 H3 cells capture 50%** of violations and **530 cover 80%** — a small, deployable target set. This is efficiency, not coincidence: exposure-adjusted vs. raw ranking correlate at Spearman **0.98**, so the top hotspots are genuine demand, not patrol artifacts.
...
| **Forecast (LightGBM Poisson, walk-forward)** | Holdout ROC-AUC / R² | **0.92** / 0.64 |
| | Holdout MAE / RMSE | 2.04 / 6.86 |
| | PAI@5% / PAI@20% | **12.7×** / 4.4× |
| | PEI@5% | 0.83 |
| | vs. baselines (last-week / rolling-7 / EWM) | Wins on MAE/RMSE/AUC; ties strong EWM on PAI |
| **Prioritization** | Hotspot cells / under-enforcement blind spots | 109 / **402** |
| | Locations for 50% / 80% coverage | **86** / 530 |
...
- **WHERE to send tow vehicles (standing pickets):** the 86-cell 50%-coverage set, led by the Central Bengaluru zone (Shivajinagar / Commercial St, ~60k) and the priority top-5 cells (ranks 1–5 around 12.97 N, 77.58 E — Upparpet/City-Market core). Top priority cell alone: 12,106 violations, Gi* z = 34.6, CIS = 100. **Action: permanent tow + picket on these 86 points = half the problem with a deployable footprint.**
...
- **No leakage:** forecast is walk-forward / time-blocked CV with a held-out month (2024-04); features are strictly lagged (lag1/7/14/28, rolling stats, neighbor lags). Holdout AUC 0.92 ≈ CV AUC 0.93 → no overfitting.
- **Honest baselines:** model is benchmarked against last-week, rolling-7, and EWM-persistence; it wins on error/AUC and **narrowly edges** the strong EWM baseline on PAI (12.73 vs 12.54) — a deliberately modest claim, because stable hotspots are inherently persistent.
...
- **Exposure-adjusted bias correction:** recorded violations reflect *enforcement activity* (patrol-shift bias), not true occurrence. We surface this via exposure-adjusted ranking (Spearman 0.98 vs. raw) and 402 detected under-enforcement blind spots (84 publicly displayable after k-anonymity).
